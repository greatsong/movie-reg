...
def load_all():
    ...
    raw = raw.drop_duplicates(subset=["영화명", "날짜"])   # 연말연초 중복 제거
...
# 곰페르츠 곡선: 영화 흥행의 비대칭 S자에 잘 맞음
# 누적(t) = M * exp(-b * exp(-k*t))
def 곰페르츠(t, M, b, k):
    return M * np.exp(-b * np.exp(-k * t))
...
# ─────────────────────────────────────────────
with tab2:
    st.caption("세 가지 방식(A·B·C)으로 각각 최종 관객을 예측하고, 2026년 몇 위 수준인지도 봅니다. "
    ...
    # ── 모델 B : 곰페르츠 곡선 (영화 흥행에 가장 적합) ──
    # 이미 증가세가 거의 멈췄으면 피팅 대신 현재값을 최종으로
    daily_recent = target.diff().dropna().iloc[-5:]
    거의정체 = (len(daily_recent) > 0) and (daily_recent.mean() < now_acc * 0.003)

    if 거의정체:
        예측결과["B · 곰페르츠 피팅"] = now_acc * 1.02   # 현재의 +2%로 마감
        curveB = None
    else:
        ...
        if len(xs) >= 6:
            try:
                popt, _ = curve_fit(
                    곰페르츠, xs, ys,
                    p0=[now_acc * 1.5, 3.0, 0.1],
                    bounds=([now_acc, 0.1, 0.005], [now_acc * 6, 50, 2.0]),
                    maxfev=10000,
                )
                M_hat = float(popt[0])
                if now_acc <= M_hat <= now_acc * 6:
                    예측결과["B · 곰페르츠 피팅"] = M_hat
                    curveB = popt
            except Exception:
                pass
    ...
    색 = {"A · 비율 기반": "orange", "B · 곰페르츠 피팅": "#4da6ff", "C · 단순 외삽": "#2ecc71"}
    ...
    if "B · 곰페르츠 피팅" in 예측결과:
        st.caption("💡 영화 흥행의 비대칭 S자 모양을 수식으로 박은 **B(곰페르츠)**가 시계열에 가장 충실해요. "
    ...
    st.caption("A는 과거 영화들의 '개봉 N일째 = 최종의 몇 %' 비율(2024~2026 학습), "
               "B는 이 영화 곡선을 곰페르츠 공식 M·exp(−b·exp(−kt))에 맞춰, "
